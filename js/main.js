...
(function () {
  ...
  function buildMirror(rows, cols) {
    ...
    var tCols = [], tRows = [], i;
    var compact = window.matchMedia('(max-height: 500px) and (orientation: landscape)').matches;
    var cellTrack = compact ? 'minmax(18px, 1fr)' : 'minmax(30px, 1fr)';
    for (i = 0; i <= cols * 2; i++) tCols.push(i % 2 === 0 ? '12px' : cellTrack);
    for (i = 0; i <= rows * 2; i++) tRows.push(i % 2 === 0 ? '12px' : cellTrack);
